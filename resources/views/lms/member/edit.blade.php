                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-xl-6 col-lg-8 col-12">
                                <form action="{{ url('members/'.$user->id) }}" method="POST" class="data-form" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    @php
                                        $assignedPermissions = DB::table('user_permission_categories')->select('user_permission_categories.*','users.type')->join('users','users.id','=','user_permission_categories.user_id')->where('user_permission_categories.user_id', $user->id)->get()->toArray();
                                       
                                        $name = collect($assignedPermissions)->pluck('name')->toArray();
                                        
                                        $subCat = collect($assignedPermissions)->pluck('sub_cat')->toArray();
                                       // dd($subCat);
                                    @endphp
                                    ...
                                    <div class="mb-3">
                                            ...
                                            <div class="form-check">
                                                <input 
                                                    ...
                                                    value="Lms" 
                                                    id="mediumLMS"
                                                    {{ isset($assignedPermissions) && in_array('Lms', $name) ? 'checked' : '' }}
                                            ...
                                            <div class="form-check">
                                                <input 
                                                    ...
                                                    value="Fms" 
                                                    id="mediumFMS"
                                                    {{ isset($assignedPermissions) && in_array('Fms', $name) ? 'checked' : '' }}
                                                    onchange="toggleSelectBox()"
                                                >
                                            ...
                                            <div class="form-check">
                                                <input 
                                                    ...
                                                    value="Cave" 
                                                    id="mediumCave"
                                                    {{ isset($assignedPermissions) && in_array('Cave', $name) ? 'checked' : '' }}
                                                    onchange="toggleSelectBox()"
                                                >
                                        ...
                                        >
                                            ...
                                            <option value="normal member" {{('normal member' == $user->type) ? 'selected' : ''}}> Normal member</option>
                                            <option value="authorized member" {{('authorized member' == $user->type) ? 'selected' : ''}}> Authorized member</option>
                                        </select>
                                        ...
                                        <label for="cave_type" id="selectBoxLabel1" style="display: none;">Cavity User Type</label>
                                            ...
                                            >
                                                <option value="" selected disabled>Select</option>
                                                <option value="custodian" {{( isset($subCat) && in_array('custodian', $subCat)) ? 'selected' : ''}}>Custodian</option>
                                                <option value="protem" {{( isset($subCat) && in_array('protem', $subCat)) ? 'selected' : ''}}>Protem</option>
                                            </select>
                                            ...
                                            >
                                                    <option value="" selected disabled>Select</option>
                                                    @foreach ($userPer as $cat)
                                                        <option value="{{$cat->id}}"  {{ collect($assignedPermissions)->contains('cus_user_id', $cat->id) ? 'selected' : '' }}> 
                                                            {{$cat->name}}
                                                        </option>
                                                    @endforeach
                                            </select>
                                    ...
                                    <!--   <select class="form-select form-select-sm" aria-label="Default select example" name="type" id="type">-->
                                    <!--                        <option value="" selected disabled>Select</option>-->
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="uploaded-files">
                                                ...
                                                <a href="{{ asset($user->image) }}" target="_blank">
                                                        ...
                                                        <span><img src="{{ asset($user->image) }}" alt="Image"></span>
...
<script>
    function toggleSelectBox() {
     ...
     document.addEventListener('DOMContentLoaded', function() {
        toggleSelectBox();  // Run once when the page loads to handle initial state
        toggleProtemSelectBox();  // Run for Protem select box based on the initial selected value
    });
</script>
